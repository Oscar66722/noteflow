...
function App() {
  ...
  // When streaming ends, load into Tiptap and set viewMarkdown
  useEffect(() => {
  ...
  const loadPdfJs = () => new Promise((resolve, reject) => {
    ...
    s.src = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.4.168/pdf.min.js'; s.async = true
  ...
  const loadJSZip = () => new Promise((resolve, reject) => {
    ...
    const s = document.createElement('script')
    s.src = 'https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js'; s.async = true
...
Output clean markdown only. Use ## for section headings, **bold** for key terms, and - for bullet points.`,
  ...
  const handleSaveNote = () => {
    ...
    setSavedNotes(updatedNotes)
    localStorage.setItem(storageKey, JSON.stringify(updatedNotes))
    setIsDirty(false); setSaveLabel('Saved')
    setTimeout(() => setSaveLabel('Save'), 2000)
  }

  const handleLoadSavedNote = (note) => {
    setNotes(note.notes); setNoteTitle(note.title)
    setIsDirty(false); setEditMode(false)
    currentNoteIdRef.current = note.id
    setSidebarOpen(false); setUploadedFiles([])
    const summary = note.summary || ''
    setViewMarkdown(summary)
    setStreamBuffer(summary)
    if (editor) {
  ...
  const handleToggleEditMode = () => {
    if (editMode && editor) {
      // Leaving edit mode — sync editor HTML back to viewMarkdown
      setViewMarkdown(editor.getHTML())
    }
    setEditMode((m) => !m)
  }
